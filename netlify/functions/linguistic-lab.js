export async function handler(event) {
    ...
    try {
        ...
        const input = body.input || body.text; // 🔥 text veya input kabul eder

        if (!mode || !input) {
            return {
                statusCode: 400,
                body: JSON.stringify({ error: "Missing mode or text" })
            };
        }
        ...
        if (!data.choices) {
            return {
                statusCode: 500,
                body: JSON.stringify({ error: "OpenAI response invalid" })
            };
        }
