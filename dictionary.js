...
text-white rounded-lg font-bold shadow-md hover:scale-105 transition-all">
Search
</button>

<div id="dictOutput" class="mt-6 bg-slate-50 dark:bg-slate-800 p-6 rounded-xl min-h-[150px]">
...
<div class="bg-white dark:bg-slate-900 p-6 rounded-xl relative max-w-lg w-full">
...
${trDef ? `<div class="text-sm text-green-600 dark:text-green-400">➜ ${trDef}</div>` : ""}
</li>
`;
}
...
class="m-1 px-2 py-1 bg-purple-200 dark:bg-purple-800 rounded cursor-pointer text-sm">
${w.word}
</span>`;
});
html += `</div>`;
...
html += `<span onclick="searchDictionaryWord('${w.word}')"
class="m-1 px-2 py-1 bg-blue-200 dark:bg-blue-800 rounded cursor-pointer text-sm">
${w.word}
</span>`;
});
html += `</div>`;
...
html += `<span onclick="searchDictionaryWord('${w.word}')"
class="m-1 px-2 py-1 bg-red-200 dark:bg-red-800 rounded cursor-pointer text-sm">
${w.word}
</span>`;
});
html += `</div>`;
...
async function renderNotebook(){
    ...
    try {
        ...
        if(!notebook || notebook.length === 0){
            container.innerHTML =
                "<p class='text-sm text-slate-400'>No saved words yet.</p>";
            return;
        }
        ...
        notebook.forEach(item => {
            ...
            div.className =
                "flex justify-between items-center bg-slate-100 dark:bg-slate-800 p-2 rounded mb-2 text-sm";
