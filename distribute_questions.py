...
questions_to_add = [
  {
    ...
    "topic": "Urban Planning / Ecology/ Environment"
  },
  {
    "type": "vocabulary",
  ...
  {
    "type": "translation",
    "question": "Eğitim sistemindeki reformlar, öğrencilerin sadece akademik başarılarını değil, aynı zamanda eleştirel düşünme becerilerini de geliştirmeyi amaçlamaktadır.",
    "choices": [
  ...
  },
  {
    ...
    "choices": [
      "It is slightly higher than the natural background rate.",
      "It is much higher than the natural background rate.",
      "It is lower than previous mass extinction events.",
      "It is difficult to estimate compared to the past.",
      "It is currently decreasing due to conservation efforts."
    ],
  ...
  },
  {
    ...
    "choices": [
      "To spread religious beliefs across Eurasia.",
      "To facilitate the exchange of goods and ideas.",
      "To establish military alliances between empires.",
      "To discover new maritime trade routes.",
      "To promote the use of a single currency."
    ],
  ...
  },
  {
    ...
    "choices": [
      ...
      "The rise of maritime trade routes.",
      "The spread of the Black Death.",
      "Frequent wars between China and Persia."
    ],
...
file_map = {
    "vocabulary": "vocab1.json",
    "sentence_completion": "sencomp1.json",
    "cloze": "cloze1.json",
    "translation": "trans1.json",
    "paraphrase": "parap1.json",
    "dialogue": "dia1.json",
    "paragraph_completion": "paracom1.json",
    "closest_meaning": "clomean1.json",
    "irrelevant_sentence": "irr1.json",
    "reading_comprehension": "read1.json"
}

category_id_map = {
    "vocab1.json": "vocab",
    "sencomp1.json": "sencomp",
    "cloze1.json": "cloze",
    "trans1.json": "trans",
    "parap1.json": "parap",
    "dia1.json": "dia",
    "paracom1.json": "paracom",
    "clomean1.json": "clomean",
    "irr1.json": "irr",
    "read1.json": "reading"
}

# Group new questions by file target
grouped = {}
for q in questions_to_add:
    fname = file_map[q["type"]]
    if fname not in grouped: grouped[fname] = []
    grouped[fname].append(q)

base_path = "public/exams/mini/special/"

for fname, q_list in grouped.items():
    ...
    current_count = data["meta"]["total_questions"]
    sec_id = category_id_map[fname]
    
    for i, nq in enumerate(q_list):
        new_id = current_count + i + 1
        
        # Convert options list to dict if needed (user provided list)
        opts = {}
        if isinstance(nq["choices"], list):
            for j, c in enumerate(nq["choices"]):
                opts[chr(65+j)] = c
        else:
            opts = nq["choices"]
        ...
        if isinstance(nq["choices"], list):
            ...
            correct_letter = nq["answer"] # Already a letter?
        ...
        # Handle Reading passages (flat for now to match current read1.json)
        if "passage" in nq:
            q_obj["passage_text"] = nq["passage"] # Extra info for frontend
            q_obj["question"] = nq["passage"] + "\n\n" + nq["question"]
            
        data["questions"].append(q_obj)
        
    data["meta"]["total_questions"] = current_count + len(q_list)
    data["meta"]["subtitle"] = f"{data['meta']['total_questions']} Soru"
    data["sections"][0]["to"] = data["meta"]["total_questions"]
    
    with open(fpath, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)

print("Distributed 40 questions.")
