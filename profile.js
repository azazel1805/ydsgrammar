...
const profileHTML = `
<div class="bg-white dark:bg-slate-900 p-6 rounded-xl shadow space-y-8">
...
<div class="mt-6">
    <input id="analyzerCodeInput"
        ...
        class="p-2 bg-slate-800 text-white rounded">
...
<!-- USER INFO -->
<div id="userInfoBox"
class="bg-slate-100 dark:bg-slate-800 p-4 rounded-lg text-sm">
Loading user...
</div>

<!-- THEME -->
<div>
<h3 class="font-bold mb-3">Theme</h3>
<button onclick="toggleDarkMode()"
class="px-4 py-2 bg-slate-200 dark:bg-slate-700 rounded">
Toggle Dark / Light
</button>
</div>
...
class="mt-3 bg-green-600 text-white py-2 px-4 rounded">
Add Note
</button>
...
class="pt-4 border-t border-slate-700 hidden">
...
async function renderProfile(){
    ...
    if(!window.currentUser){
        ...
        userBox.innerHTML = "<span class='text-slate-400'>Not logged in.</span>";
...
async function renderSavedWords(){
    ...
    try {
        ...
        words.forEach(item => {
            ...
            div.className =
                "flex justify-between items-center bg-slate-200 dark:bg-slate-700 p-2 rounded mb-2 text-sm";
